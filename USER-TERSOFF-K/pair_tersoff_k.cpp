...
PairTersoff_k::PairTersoff_k(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
  restartinfo = 0;
  one_coeff = 1;
...
void PairTersoff_k::compute_pre_qeq(int eflag, int vflag)
{
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    for (int jj = 0; jj < sht_jnum; jj++) {
      ...
      double prefactor, fpair_divfq, evdwl_divfq;
      force_zeta(&params[iparam_ij],rsq1,zeta_ij,fpair_divfq,prefactor,1,evdwl_divfq);
      Sht.ijpage[sht_first+jj].prefactor = prefactor;
...
void PairTersoff_k::compute(int eflag, int vflag)
{
  ...
  for (int ii = 0; ii < inum; ii++) {
    ...
    double e_self = e_self_wolf+e_self_chi;
    ...
    for (int jj = 0; jj < sht_jnum; jj++) {
      ...
      for (int kk = 0; kk < sht_jnum; kk++) {
        ...
        double prefactor = Sht.ijpage[sht_first+jj].prefactor;
        prefactor *= ters_fq(&params[iparam_ijk], q[i]);
...
void PairTersoff_k::settings(int narg, char **arg)
{
...
void PairTersoff_k::coeff(int narg, char **arg)
{
  int i,j,n;
...
void PairTersoff_k::init_style()
{
  if (atom->tag_enable == 0)
    error->all(FLERR,"Pair style Tersoff/k requires atom IDs");
  if (force->newton_pair == 0)
    error->all(FLERR,"Pair style Tersoff/k requires newton pair on");
  if (!atom->q_flag)
...
double PairTersoff_k::init_one(int i, int j)
{
...
void PairTersoff_k::read_file(char *file)
{
  const int params_per_line_max = 29;
  ...
  while (1) {
    ...
    if (nwords != params_per_line){
      error->all(FLERR,"Incorrect format in Tersoff/k potential file (different number of parameters)");
    }
      ...
      case SINGLE:
        {
          ...
          tmp_single.el_chi = atof(words[2]);
          tmp_single.el_eta = atof(words[3]);
          tmp_single.ni_newtral = atof(words[4]);
          tmp_single.ni0 = atof(words[5]);
      ...
      case DOUBLE:
        {
          ...
          tmp_double.powern_del = atof(words[16]);
          tmp_double.powern_rev = atof(words[17]);
          tmp_double.powern_rev_del = atof(words[18]);
          tmp_double.Q = atof(words[19]);
          tmp_double.bigre = atof(words[20]);
          tmp_double.bigr = atof(words[21]);
          tmp_double.bigd = atof(words[22]);
          tmp_double.w_alpha = atof(words[23]);
          tmp_double.coulcut = atof(words[24]);
          tmp_double.gamma = atof(words[25]);
...
void PairTersoff_k::coulomb_interaction(const Param* const param, const double rsq, const double factor_coul, double &fvionij_divq, const int eflag, double& eng_divq){
  const double alf = param->w_alf;
  const double e_shift = param->e_shift;
  const double f_shift = param->f_shift;
...
void PairTersoff_k::repulsive(Param* const param, const double rsq, double &fforce,
...
double PairTersoff_k::zeta(Param* const param, const double rsqij, const double rsqik,
                         double *delrij, double *delrik)
{
  ...
  costheta = (delrij[0]*delrik[0] + delrij[1]*delrik[1] +
	...
	double rdiff = (rij-param->bigre_ij)-(rik-param->bigre_ik);
  if (param->powermint == 3) arg = param->alpha*pow(rdiff,3.0);
  else arg = param->alpha * rdiff;

  if (arg > 69.0776) ex_delr = 1.e30;
  else if (arg < -69.0776) ex_delr = 0.0;
  else ex_delr = exp(arg);

  return ters_fc(1, rik,param) * ters_gijk(costheta,param) * ex_delr;
}

/* ---------------------------------------------------------------------- */

void PairTersoff_k::force_zeta(Param* const param, const double rsq, const double zeta_ij, double &fforce, double &prefactor,
...
void PairTersoff_k::attractive(Param* const param, const double prefactor,
...
double PairTersoff_k::ters_fc(const int ikflag, const double r, Param *param)
{
...
double PairTersoff_k::ters_fc_d(const int ikflag, const double r, Param *param)
{
  double exp_corr = param->cutoff_corr_ij;
  double exp_r0 = param->cutoff_r0_ij;
  double ters_R = param->bigr_ij;
...
double PairTersoff_k::ters_fa(const double r, Param *param)
{
...
double PairTersoff_k::ters_fa_d(const double r, Param *param)
{
  if (r > param->bigr_ij + param->bigd_ij) return 0.0;
...
double PairTersoff_k::ters_bij(const double zeta, Param *param)
{
  double tmp = zeta;
...
double PairTersoff_k::ters_bij_d(const double zeta, Param *param)
{
  double tmp = zeta;
  if (tmp > param->ca1){
...
void PairTersoff_k::ters_zetaterm_d(const double prefactor,
                                  ...
                                  Param* const param)
{
  ...
  dfc = ters_fc_d(1, rik,param);
	...
	double rdiff = (rij-param->bigre_ij)-(rik-param->bigre_ik);
  if (param->powermint == 3) tmp = param->alpha*pow(rdiff,3.0);
  else tmp = param->alpha * rdiff;
  ...
  if (param->powermint == 3)
    ex_delr_d = 3.0*param->alpha * pow(rdiff,2.0)*ex_delr;
  else ex_delr_d = param->alpha * ex_delr;
...
void PairTersoff_k::costheta_d(double *rij_hat, double rij,
...
double PairTersoff_k::get_chi_i(int i)
{
  return pre_qeq_i[i].chi;
}

double PairTersoff_k::get_bigj_ii(int i)
{
  return pre_qeq_i[i].bigj_ii;
}

const sparse_matrix& PairTersoff_k::get_H() const
{
  return H;
}
